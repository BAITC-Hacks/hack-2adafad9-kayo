...
Время полного прогона: 18 с на ноутбуке (валидация, обучение, конформная калибровка коридора,
...
Архив прогнозов погоды лежит в репозитории (`data/weather/*.parquet`, около 550 КБ), поэтому прогон
воспроизводится без интернета.
...
3. **Рефлексия агента** — перед публикацией прогноза агент сверяет свою p50 с фактом последних семи
   суток и подкручивает смещение по каждой турбине. Это и есть замкнутая петля обратной связи: не
   переобучать всю модель, а компенсировать локальный дрейф.
...
| --- | --- | --- | --- | --- |
| Наше решение | 0,175 | 0,175 | 0,54 | 0,54 |
...
Покрытие коридора P10–P90: **79 %** (цель — около 80 %). Достигается конформной калибровкой:
последние 30 суток обучения не идут в fit, а служат для оценки конформного отступа. Отдельно на
пару «турбина × горизонт», потому что у 48 ч разброс шире, чем у 24 ч.
...
- Ансамбль погодных моделей (ICON, GFS, ECMWF) вместо одной: расхождение между ними уже считается как
  мера неопределённости, следующий шаг — отдельные признаки по каждой модели.
- Коррекция смещения по вчерашним ошибкам: агент разбирает прошедшие сутки и подстраивает прогноз.
